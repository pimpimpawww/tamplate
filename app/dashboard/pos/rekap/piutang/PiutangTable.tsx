...
export function PiutangTable({ data }: { data: Row[] }) {
    ...
    <div className="space-y-4">
      ...
      {search && (
        <div className="p-3 rounded-lg bg-amber-50 border border-amber-200">
          <p className="text-xs text-amber-700">Total piutang dari hasil pencarian: <span className="font-bold">{formatRupiah(total)}</span></p>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-gray-50 text-muted-foreground text-xs uppercase tracking-wide">
              ...
              <th className="text-left px-4 py-3">Tgl Tagih</th>
            ...
            {filtered.map(t => (
              <tr key={t.id} className="border-b hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-muted-foreground">
                  {t.tanggalTagih ? new Date(t.tanggalTagih).toLocaleDateString('id-ID') : '-'}
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-right font-semibold" style={{ color: '#d97706' }}>
