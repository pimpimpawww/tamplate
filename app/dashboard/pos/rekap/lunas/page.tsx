...
import { LunasTable } from './LunasTable'

function formatRupiah(n: number | string) {
  return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(Number(n))
}

export default async function RiwayatLunasPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">Riwayat Pendapatan</h1>
          <p className="text-sm text-muted-foreground">{termins.length} transaksi lunas</p>
        </div>
      </div>
      <div className="p-4 rounded-lg" style={{ background: '#1e2328' }}>
        <p className="text-xs" style={{ color: '#a8b89a' }}>Total Pendapatan Lunas</p>
        <p className="text-2xl font-bold" style={{ color: '#86efac' }}>{formatRupiah(total)}</p>
      </div>
      <LunasTable data={termins.map(t => ({
        id: t.id,
